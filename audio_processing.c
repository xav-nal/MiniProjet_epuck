	...
	static uint8_t mustSend = 0;
		...
		if(nb_samples >= (2 * FFT_SIZE)){
			...
			*/
			 ...
			 *
			*/
			for(uint16_t i = FREQ_COUPURE + 1 ; i < FFT_SIZE/2 ; i++){
				micRight_cmplx_input[i] = 0;
				micLeft_cmplx_input[i] = 0;
			...
			//uint16_t highest_pic_L = sound_remote_new(micLeft_output);
				...
				for(uint16_t i = 0 ; i < FFT_SIZE/2 ; i++) {
					if((micRight_cmplx_input[i] =! 0) && (micLeft_cmplx_input[i] =! 0)){
						double angle_R = atan2(micRight_cmplx_input[2*i+1], micRight_cmplx_input[2*i]);
						double angle_L = atan2(micLeft_cmplx_input[2*i+1], micLeft_cmplx_input[2*i]);
